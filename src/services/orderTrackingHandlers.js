...
// Helpers
// -------------------------
const normalizeToBinanceSymbol = symbol => {
  if (!symbol) return '';
  return symbol.endsWith('USDT') ? symbol : symbol.replace('USD', 'USDT');
};

const getInternalSymbol = symbol => normalizeToBinanceSymbol(symbol);

const isBinanceFuturesSymbol = symbol => 
  typeof symbol === 'string' && symbol.endsWith('USDT');

// -------------------------
// WebSocket Broadcast Helpers
// -------------------------
const broadcastSymbolUpdate = (connections, type, symbol, data) => {
...
// Public Functions
// -------------------------
export const subscribeSymbol = (req, res) => {
  let { symbol } = req.body;
  const connections = req.app.get('orderTrackingConnections');
...
export const unsubscribeSymbol = (req, res) => {
  let { symbol } = req.body;
  const connections = req.app.get('orderTrackingConnections');
...
export const cancelOrderTrackingWss = (req, res) => {
  const connections = req.app.get('orderTrackingConnections');
  console.log(`Closing ${connections.size} orderTracking WebSocket connections...`);
...
export const getOrderTrackingWss = (req, res) => {
  const connections = req.app.get('orderTrackingConnections');
  res.send({ activeConnections: connections.size });
};

export const broadcastAllTrackedSymbols = (req, res) => {
  const connections = req.app.get('orderTrackingConnections');
...
export const broadcastOrderTracking = (symbol, connections, symbolData = null) => {
  const normalized = normalizeToBinanceSymbol(symbol);
  ...
  if (isFutures) {
    const { mark_price, quotes } = rawData;
    markPrice = parseFloat(mark_price || quotes?.mark_price || 0);
  } else if (isOption) {
    const { calculated, originalData } = rawData;
    markPrice = parseFloat(
      calculated?.mark_price?.value ??
      originalData?.mark_price ??
      originalData?.last_price ??
      0
    );
  } else {
    console.log(`[OrderTracking] Unknown symbol type for: ${normalized}`);
